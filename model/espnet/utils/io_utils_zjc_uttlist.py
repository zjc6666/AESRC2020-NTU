...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch):
        ...
        z_feats_dict = OrderedDict()  # 存放xvector
        uttid_list = []  # List[str]
        ...
        for uttid, info in batch:
            ...
            if self.load_input:
                ...
                for idx, inp in enumerate(info['input']):
                    ...
                    # z = self._get_from_loader(
                    #     filepath=inp["xvector"], filetype=inp.get("filetype", "mat")
                    # )
                    # z_feats_dict.setdefault(inp["name"], []).append(z)
        ...
        if self.mode == 'asr':
            return_batch, uttid_list = self._create_batch_asr(
                x_feats_dict, y_feats_dict, z_feats_dict, uttid_list)
        elif self.mode == 'tts':
            _, info = batch[0]
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict, z_feats_dict, uttid_list):
        ...
        zs = [[x[i] for i in nonzero_sorted_idx] for x in zs]
        # print("####ZJC xs2==>",xs)
        ...
        if self.load_output:
            ...
            return_batch = OrderedDict([*[(x_name, x) for x_name, x in zip(x_names, xs)], (y_name, ys), ('input3', uttid_list)])
        else:
