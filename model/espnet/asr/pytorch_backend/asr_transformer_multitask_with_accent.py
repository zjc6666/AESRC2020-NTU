...
from espnet.utils.io_utils_zjc_uttlist import LoadInputsAndTargets
...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        # 为了查看batch的内容
        # print("####ZJC utt_list==>",utt_list)

        # print("#####ZJC batch[0].length==>",len(batch[0]))
        # print("#####ZJC batch[0]==>",batch[0])
        # print("#######ZJC X.length==>",len(xs))
        # print("#######ZJC X.shape==>",xs[0])
        # print("#######ZJC X==>",xs)
        # print("#######ZJC Y.length==>",len(ys))
        # print("#######ZJC Y.shape==>",ys[0])
        # print("#######ZJC Y==>",ys)
...
def train(args):
    ...
    with open(args.valid_json, "rb") as f:
        valid_json = json.load(f)["utts"]
    # print("####ZJC train_json.length==>",len(train_json)) 
    # print("####ZJC train_json==>",train_json)
    ...
    # print("####ZJC train make_batchset.length==>",len(train))
    # print("####ZJC train make_batchset==>",train)
    valid = make_batchset(
        valid_json,
        args.batch_size,
        args.maxlen_in,
    ...
    #print("#####ZJC trian_iter.length==>",len(train_iter))
    # print("#####ZJC trian_iter==>",train_iter)
    # print("####ZJC converter(load_cv)==>",converter([load_cv(valid)]))
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:                                                 
                    ...
                    listEmb = model.extract_embedding(feat)
                    new_js[name] = listEmb

    # for lay in range(12):
    with open(args.result_label + "_layer" + ".txt", "w") as f:
        for utt, emb in new_js.items():
            f.write(utt + " " + arrToStr(emb.tolist()) + "\n")
...
def recog(args):
    ...
    # logging.info("@ZJC asr.py recog is called args: %s" , str(args.batchsize))
    if args.streaming_mode and "transformer" in train_args.model_module:
        raise NotImplementedError("streaming mode for transformer is not implemented")
    logging.info(
        " Total parameter of the model = "
    ...
    if(args.soft_weight_output != ""):
        weight_file = open(args.soft_weight_output, 'w')
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    elif(args.recog_mode=="accent"):
                        logging.warning("recog_mode accent ==> accent,name = " + str(name))
                        nbest_hyps = model.recognize_accent(
                           feat, name,args, train_args.char_list, rnnlm
                        )
                    else:
                        logging.warning("Recog-mode is not recognition, not accent or asr")
                        sys.exit(1)
                # print("#### nbest_hyps===> ",nbest_hyps)
                if(args.soft_weight_output != ""):
                    att_weight = str(np.array(att_weight)).replace("\n", "")
                    weight_file.write(name + " " + att_weight + "\n")
            ...
            for names in grouper(args.batchsize, keys, None):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    nbest_hyps = model.recognize_batch(
                        feats, args, names, train_args.char_list, rnnlm=rnnlm
                    )
                for k,v in nbest_hyps.items():
                    name = k
                    a = list()
                    a.append(v)
                    v = [-1] + a
                    ...
                    hyp['score'] = -1
                    new_js[name] = add_results_to_json(
                        js[name], [hyp], train_args.char_list
                    )
